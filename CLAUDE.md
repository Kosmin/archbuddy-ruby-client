# CLAUDE.md — archbuddy-ruby-client
...
1. This file (`CLAUDE.md`) — stack, invariants, vocabulary, task workflow
...
3. [`APP_SCHEMA.md`](APP_SCHEMA.md) — the **contract/schema** doc: what the collector EMITS
   (`graph.yml` + `id-map.yml` shapes) and what `report` CONSUMES (`findings.yml` shape), by reference to
   the engine's canonical contract. (This repo has **no database**; APP_SCHEMA documents the data
   contracts instead.)
...
□ Changed the Anonymizer / graph / id-map shape  → ARCHITECTURE.md + APP_SCHEMA.md (emitted shapes)
...
□ Changed what `report` consumes (findings shape)→ APP_SCHEMA.md (consumed shape) — coordinate with the engine repo
...
□ Changed the metric set                         → report.rb constant + the engine's METRIC_KEYS (lockstep) + APP_SCHEMA.md
□ Introduced a new pattern/convention/invariant  → this file (CLAUDE.md)
```
