...
# v0.15 P1-T5: the re-based evaluability battery, grown to the EIGHT-rule
# family at v0.16 T8 ([D1] §P1-T5 — replaces the substrate v1/v4/v5 × 9-rule
# cross-product). Grows along the rule-task chain: each rule task adds its
# rows as its class registers; the fabrication guard below is family-generic
# from day one.
#
#   v1  (6-key nodes, no edges): ExponentialNode evaluates; UCC/FB/UCD →
#        `fragments carry no edges`; zero fabricated findings.
#   v6-shaped (edges + escapes + outcome_arity + score stamps): ALL eight
#        evaluate — zero N/A (X-12: the v0.15 v4 stub extended with the
#        score triple so the zero-N/A form survives the 8th rule).
...
RSpec.describe "Rule-family evaluability battery" do
  ...
  it "v1: ReusabilityScore honestly declares the pinned score-keys-absent reason" do
    result = evaluate(v1_vintage)
    expect(result.not_evaluable).to include(
      rule: "ReusabilityScore",
      reason: "score stamps absent — serializer < v6 or vintage never analyzed"
    )
    expect(result.findings.map(&:rule)).not_to include("ReusabilityScore")
  end
  ...
  it "ships base + exactly the EIGHT rule class files (no NoNew* files)" do
    ...
    expect(files).to eq(%w[
      ...
      multiplicative_growth.rb reusability_score.rb review_surface.rb
      use_case_complexity.rb use_case_dividend.rb
    ])
  end

  it "v6-shaped stub (edges + escapes + outcome_arity + score stamps): ALL eight " \
     "evaluate — zero N/A" do
    # X-12: the v0.15 v4 stub extended with the FRESH engine score triple
    # (collapse == round(branches/max(arity,1), 2)) — proves the 8-rule
    # zero-N/A form; a score-keys-absent stub would emit the T8 note instead.
    ep = ReviewStubs.stub_node(file: "app/api/a.rb", symbol: "A#GET[0]", branches: 4,
                               entrypoint: true, entrypoint_kind: "api",
                               escapes: false, outcome_arity: 2, collapse: 2.0,
