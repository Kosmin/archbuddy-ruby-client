...
# Per-target external sub-sinks (v0.11 E1, L13; category-era record W2-C).
# The ONE historical `<external>` sink is sub-classified into per-target
# sinks (`<external:{category}:{const_fq}>`, one per distinct provable
# [category, target] pair) plus the always-minted generic `<external>` —
# ALL still kind:"external" (closed 4-kind vocab untouched, I6). Each
# per-target sink carries the OPTIONAL RawNode#terminal_kind stamp = the
# CATEGORY word (never the target; the sink-side twin of entrypoint_kind);
# the generic sink carries NONE. Graph emission of terminal_kind is GATED on
# the installed engine schema (a 1.2 engine REJECTS unknown node keys — the
# same verified posture as entrypoint_kind / M4).
RSpec.describe "Egress category sinks (W2-C)" do
  ...
  it "stamps terminal_kind (the CATEGORY word) on per-target sinks and NOT on the generic sink (CR-5)" do
    in_repo(MULTI_CATEGORY_SRC) do |dir|
      raw   = collect(dir)
  ...
  it "routes each external edge to its [category, target] sink" do
    in_repo(MULTI_CATEGORY_SRC) do |dir|
      result = anonymize(dir)
  ...
  it "holds terminal_kind OFF the graph node while the engine schema rejects the key" do
    in_repo(MULTI_CATEGORY_SRC) do |dir|
      result  = anonymize(dir)
      http_id = id_for(result, "<external:http:Faraday>")
