...
|--------------|--------|----------------|
| `formatter.rb` (`Formatter`, `RenderContext`) | — | **R-6 strategy base + open/closed `FORMATS` registry.** `register(name, klass)` / `for(name)`. Receives an already-de-anonymized, already-ranked `RenderContext(ranked, class_rollups, generator, graph, resolver, scores, connectivity, max_nodes, multiplexer_proxies, entrypoints, egress, dynamic_dispatch, blast_radius, forward_depth, reverse_depth, branching_factor)`; makes ZERO analytic decisions. `scores` is the optional R-8 dimension-scores model (`nil` for a 1.0 doc). `connectivity` is the optional V8 `Scores::Connectivity` struct (`nil` for 1.0/1.1/1.2 docs — keyword_init, nil-default, backward-compatible); the three v0.10 counter blocks and the four v0.11 business-metric fields (`blast_radius`/`forward_depth`/`reverse_depth`/`branching_factor` — flat, C1) follow the same nil-default pattern and feed the Business Impact presenter. Eager-requires the five built-ins so registration happens on load. |
...
| `spec/cache/egress_coverage_aggregate_spec.rb` | **v0.10 W3 committed counter blocks** (stamp literals re-baselined to SERIALIZER v3 in v0.11): `entrypoints` (closed category set seeded to 0, `unknown` bucket, null mean/median until analyze), `egress` (from `diagnostics[:egress_counts]`; generic graph-fold fallback), `dynamic_dispatch` (coverage_ratio, NULL on zero denominator); diagnostics-free write carries prior blocks forward VERBATIM; fragment nodes carry `entrypoint_kind`. |
| — v0.11 W-C v3 battery (in `egress_coverage_aggregate_spec.rb` + `writer_spec.rb`) | **SERIALIZER v3**: verbatim 1.6 folds (flat spellings, de-anonymized blast worst), `headline_scores` median/median_grade/capped_fraction (the v2 median-gap fix), egress cost graft, carry-forward retention across collect-only rewrites (blocks byte-identical, counts fresh / cost carried), v2-prior no-manufacture; the `writer_spec` preserve example extended to the four blocks. |
| `spec/report/business_impact_spec.rb` | **v0.11 W-C T6/T9 `Report::BusinessImpact`**: the six pinned verbatim templates (Q1–Q5 + Branching footer), q3 worked example byte-exact, cap-note thresholds (0 / 0<f<0.5 / ≥0.5 → "median at cap"), per-question omission, `[]` on all-nil, and the seven-row L17 nil-tolerance matrix (v1 no-scores / v1+scores / v2 / v3-over-1.5 / v3-over-1.6 / legacy opaque 1.6 + id-map / engine N/A blast form) driven through the REAL parsers. |
