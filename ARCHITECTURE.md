...
| `lib/archbuddy/version.rb` | `Archbuddy::VERSION` (`"0.9.0"`, the v0.10 ingress/egress release). |
...
| `spec/collect/egress_sinks_spec.rb` | **v0.10 W2-C category sinks**: `<external:http\|gem\|queue>` minted only when the category appears; generic `<external>` always minted (byte-identical no-egress back-compat); all `kind:"external"`; `terminal_kind` in the id-map; edges route per category. |
