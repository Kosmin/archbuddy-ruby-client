...
| `ruby/vocab.rb` (`Vocab`) | Pure static vocabularies the resolver consults: `OPERATOR_DENY` (D36), `METAPROGRAMMING`, `ACTIVE_RECORD` query/persistence methods, `ACTIVE_RECORD_BASES`, `CONTROLLER_BASES`. Also declares `AR_WRITE`/`AR_DESTROY`/`ar_op_kind` for the V4/P4 `sink_open` classifier (read/write/destroy partition). |
